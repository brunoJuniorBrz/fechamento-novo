...
export default function CashierClosingPage() {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (currentUser) {
        const { data: profileData, error: profileError } = await supabase
          ...
          .maybeSingle();
        if (profileError) {
          console.error("Error fetching profile for FechamentoPage:", profileError.message);
          ...
          if (profileData) {
            ...
            if (profileData.loja_id && profileData.loja_id !== 'admin') { // Simone (admin) might not have pending receivables for "her" store
  ...
  const handleSaveCashier = async () => {
    ...
    const fechamentoData: Omit<NewClosingData, 'saidas_operacionais_loja' | 'simone_saidas_operacionais' | 'novas_contas_a_receber' | 'pagamentos_recebidos'> = {
        ...
        entradas: Object.fromEntries(entrances.map(e => [e.id, parseInput(e.quantity)]).filter(([,qty]) => qty > 0)),

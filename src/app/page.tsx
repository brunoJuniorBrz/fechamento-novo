...
export default function HomePage() {
  ...
  React.useEffect(() => {
    console.log('[DEBUG Home] useEffect iniciado');
    // Checagem inicial da sessão
    const checkInitialSession = async () => {
      setAuthLoading(true);
      console.log('[DEBUG Home] Iniciando checkInitialSession');
      const { data: { session } } = await supabase.auth.getSession();
      console.log('[DEBUG Home] getSession retornou:', session);
      const currentUser = session?.user ?? null;
      setUser(currentUser);
      if (currentUser) {
        console.log('[DEBUG Home] Usuário autenticado:', currentUser.id);
        const { data: profileData, error: profileError } = await supabase
          .from('profiles')
          .select('loja_id, admin, nome_operador')
          .eq('id', currentUser.id)
          .maybeSingle();
        console.log('[DEBUG Home] Resultado do fetch de perfil:', { profileData, profileError });
        if (profileError) {
          setProfile(null);
          ...
          if (profileData.admin) {
            console.log('[DEBUG Home] Usuário admin, redirecionando para /admin');
            router.replace('/admin');
            setAuthLoading(false);
            return;
          } else if (profileData.loja_id) {
            console.log('[DEBUG Home] Usuário comum, buscando fechamentos da loja:', profileData.loja_id);
        ...
        setProfile(null);
        console.warn('[DEBUG Home] Nenhum usuário autenticado, redirecionando para /login');
        router.replace('/login');
        setAuthLoading(false);
        return;
      }
      setAuthLoading(false);
      console.log('[DEBUG Home] checkInitialSession finalizado');
    };
    checkInitialSession();
    // Listener para mudanças futuras de autenticação
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      setAuthLoading(true);
      console.log('[DEBUG Home] onAuthStateChange disparado:', event, session);
      const currentUser = session?.user ?? null;
      setUser(currentUser);
      if (currentUser) {
        const { data: profileData, error: profileError } = await supabase
          ...
          .eq('id', currentUser.id)
          .maybeSingle();
        console.log('[DEBUG Home] onAuthStateChange perfil:', { profileData, profileError });
        if (profileError) {
          setProfile(null);
          toast({variant: "destructive", title: "Erro de Conexão com Perfil", description: `Não foi possível buscar o perfil: ${profileError.message}`});
          setClosingsLoading(false);
    ...
    return () => { authListener.subscription.unsubscribe(); console.log('[DEBUG Home] Listener de auth removido'); };

...
export default function HistoryDatePage() {
    ...
    React.useEffect(() => {
        console.log('[DEBUG Historico] useEffect iniciado');
    ...
    React.useEffect(() => {
        ...
        const checkInitialSession = async () => {
            setAuthLoading(true);
            console.log('[DEBUG Historico] Iniciando checkInitialSession');
            const { data: { session } } = await supabase.auth.getSession();
            console.log('[DEBUG Historico] getSession retornou:', session);
            const currentUser = session?.user ?? null;
            setUser(currentUser);
            if (currentUser) {
                console.log('[DEBUG Historico] Usuário autenticado:', currentUser.id);
                const { data: profileData, error: profileError } = await supabase
                    .from('profiles')
                    .select('id, loja_id, nome_operador, admin')
                    .eq('id', currentUser.id)
                    .maybeSingle();
                console.log('[DEBUG Historico] Resultado do fetch de perfil:', { profileData, profileError });
                if (profileError) {
                    console.error('[DEBUG Historico] Erro ao buscar perfil:', profileError.message);
                    setProfile(null);
                ...
                if (dateParam) {
                    try {
                        ...
                        console.log('[DEBUG Historico] Buscando dados do fechamento:', dateObject, profileData?.loja_id, profileData?.admin);
                ...
                setProfile(null);
                console.warn('[DEBUG Historico] Nenhum usuário autenticado, redirecionando para /login');
                router.push('/login');
                setAuthLoading(false);
                return;
            }
            setAuthLoading(false);
            console.log('[DEBUG Historico] checkInitialSession finalizado');
        };
        checkInitialSession();
        // Listener para mudanças futuras de autenticação
        const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
            setAuthLoading(true);
            console.log('[DEBUG Historico] onAuthStateChange disparado:', event, session);
            const currentUser = session?.user ?? null;
            setUser(currentUser);
            if (currentUser) {
                const { data: profileData, error: profileError } = await supabase
                    ...
                    .eq('id', currentUser.id)
                    .maybeSingle();
                console.log('[DEBUG Historico] onAuthStateChange perfil:', { profileData, profileError });
                if (profileError) {
                    setProfile(null);
                    toast({ variant: "destructive", title: "Erro de Perfil", description: "Não foi possível carregar dados do perfil."});
                    router.push('/login');
        ...
        return () => { authListener.subscription.unsubscribe(); console.log('[DEBUG Historico] Listener de auth removido'); };

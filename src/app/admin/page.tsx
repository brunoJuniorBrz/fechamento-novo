...
import { BarChart, Filter, DollarSign, TrendingUp, TrendingDown, Building, Car, Truck, Bike, FileSearch, ClipboardPen, CreditCard, QrCode, Banknote, UserMinus, Receipt, Package, CheckCircle, ListChecks, type LucideIcon, Wallet, Landmark, Hourglass, TrendingDownIcon, AlertTriangle, ReceiptText, User as UserIconLc, CircleDollarSign, Coins } from 'lucide-react';
        ...
        {payload.map((item) => {
          ...
          return (
            <div key={name} className="flex items-center gap-1.5">
              <span className="h-2.5 w-2.5 shrink-0 rounded-[2px]" style={{ backgroundColor: item.color, opacity: item.payload?.fillOpacity ?? 1 }} />
...
export default function AdminDashboardPage() {
    ...
    React.useEffect(() => {
      ...
      console.log('[DEBUG Admin] useEffect iniciado');
      // 1. Checagem inicial da sessão
      const checkInitialSession = async () => {
        setAuthLoading(true);
        console.log('[DEBUG Admin] Iniciando checkInitialSession');
        const { data: { session } } = await supabase.auth.getSession();
        console.log('[DEBUG Admin] getSession retornou:', session);
        const currentUser = session?.user ?? null;
        setUser(currentUser);
        if (currentUser) {
          console.log('[DEBUG Admin] Usuário autenticado:', currentUser.id);
          const { data: profileData, error: profileError } = await supabase
            .from('profiles')
            .select('id, loja_id, nome_operador, admin')
            .eq('id', currentUser.id)
            .maybeSingle();
          console.log('[DEBUG Admin] Resultado do fetch de perfil:', { profileData, profileError });
          if (profileError) {
            console.error('[DEBUG Admin] Erro ao buscar perfil:', profileError.message);
            ...
            if (!profileData?.admin) {
              console.warn('[DEBUG Admin] Usuário não é admin, redirecionando para /');
              toast({ variant: "destructive", title: "Acesso Negado", description: "Você não tem permissão para acessar esta página." });
              router.push('/');
              setAuthLoading(false);
              return;
            } else {
              console.log('[DEBUG Admin] Perfil admin carregado:', profileData);
          ...
          console.warn('[DEBUG Admin] Nenhum usuário autenticado, redirecionando para /login');
          setProfile(null);
          router.push('/login');
          setAuthLoading(false);
          return;
        ...
        setAuthLoading(false);
        console.log('[DEBUG Admin] checkInitialSession finalizado');
      };
      checkInitialSession();
      // 2. Listener para mudanças futuras de autenticação
      const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
        setAuthLoading(true);
        console.log('[DEBUG Admin] onAuthStateChange disparado:', event, session);
        const currentUser = session?.user ?? null;
        setUser(currentUser);
        if (currentUser) {
          const { data: profileData, error: profileError } = await supabase
            ...
            .eq('id', currentUser.id)
            .maybeSingle();
          console.log('[DEBUG Admin] onAuthStateChange perfil:', { profileData, profileError });
          if (profileError) {
            setProfile(null);
            router.push('/login');
            setAuthLoading(false);
      ...
      return () => {
        ...
        console.log('[DEBUG Admin] Listener de auth removido');
      };
            ...
            <div className="flex flex-col min-h-screen bg-gradient-to-b from-background to-muted/30">
                ...
                <main className="flex-grow container mx-auto px-4 md:px-8 py-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                        <Card className="shadow-md border border-border/50"><CardHeader><CardTitle>Entradas Comuns e Recebimentos</CardTitle></CardHeader><CardContent className="aspect-video">{dataLoading ? <Skeleton className="w-full h-full" /> : entradasComunsChartData.length > 0 ? (<ChartContainer config={entradaChartConfig} className="min-h-[200px] w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel hideIndicator formatter={(v,n)=>(<div className="flex items-center justify-between w-full min-w-[150px]"><span className="text-muted-foreground mr-2">{n}:</span><span className="font-bold">{formatCurrency(v as number)}</span></div>)} />} /><Pie data={entradasComunsChartData} dataKey="total" nameKey="name" innerRadius={60} outerRadius={90} paddingAngle={2}>{entradasComunsChartData.map((entry, index) => ( <Cell key={`cell-entrada-${index}`} fill={entry.fill} /> ))}</Pie><Legend content={<ChartLegendContent />} /></PieChart></ResponsiveContainer></ChartContainer>) : ( <p className="text-center text-muted-foreground py-10">Sem dados para o gráfico.</p> )}</CardContent></Card>
                        <Card className="shadow-md border border-border/50"><CardHeader><CardTitle>Entradas Eletrônicas</CardTitle></CardHeader><CardContent className="aspect-video">{dataLoading ? <Skeleton className="w-full h-full" /> : entradasEletronicasChartData.length > 0 ? (<ChartContainer config={entradaEletronicaChartConfig} className="min-h-[200px] w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel hideIndicator formatter={(v,n)=>(<div className="flex items-center justify-between w-full min-w-[150px]"><span className="text-muted-foreground mr-2">{n}:</span><span className="font-bold">{formatCurrency(v as number)}</span></div>)} />} /><Pie data={entradasEletronicasChartData} dataKey="total" nameKey="name" innerRadius={60} outerRadius={90} paddingAngle={2}>{entradasEletronicasChartData.map((entry, index) => ( <Cell key={`cell-eletronica-${index}`} fill={entry.fill} /> ))}</Pie><Legend content={<ChartLegendContent />} /></PieChart></ResponsiveContainer></ChartContainer>) : ( <p className="text-center text-muted-foreground py-10">Sem dados para o gráfico.</p> )}</CardContent></Card>
                        <Card className="shadow-md border border-border/50 lg:col-span-2"><CardHeader><CardTitle>Saídas e Novas A/R</CardTitle></CardHeader><CardContent className="aspect-video">{dataLoading ? <Skeleton className="w-full h-full" /> : saidasChartData.length > 0 ? (<ChartContainer config={saidasChartConfig} className="min-h-[200px] w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel hideIndicator formatter={(v,n)=>(<div className="flex items-center justify-between w-full min-w-[150px]"><span className="text-muted-foreground mr-2">{n}:</span><span className="font-bold">{formatCurrency(v as number)}</span></div>)} />} /><Pie data={saidasChartData} dataKey="total" nameKey="name" innerRadius={60} outerRadius={90} paddingAngle={2}>{saidasChartData.map((entry, index) => ( <Cell key={`cell-saida-agg-${index}`} fill={entry.fill} /> ))}</Pie><Legend content={<ChartLegendContent />} /></PieChart></ResponsiveContainer></ChartContainer>) : ( <p className="text-center text-muted-foreground py-10">Sem dados para o gráfico.</p> )}</CardContent></Card>
                    </div>
